...
Knob.prototype = {
  ...
  _updateWhileMoving: function (event) {
    ...
    var deg =
      (Math.atan2(-y, -x) * 180) / Math.PI + 90 - this.settings.angleoffset;
    var percent;
  ...
  changed: function (direction) {
    this.input.value = this.limit(
      parseFloat(this.input.value) + direction * (this.input.step || 1)
    );
...
Ui.El.Arc = function (options) {
  ...
  this.options.angleoffset =
    (options.angleoffset || 0) - (this.options.labels ? 0 : 90);
...
Ui.El.Arc.prototype.getCoords = function (angle) {
  ...
  var innerRadius =
    this.options.innerRadius || this.options.width / 2 - this.options.arcWidth;
